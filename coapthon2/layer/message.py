...
class MessageLayer(object):
    ...
    def matcher_response(self, response):
        if response.mid is None:
            response.mid = self._parent._currentMID % (1 << 16)
            self._parent._currentMID += 1
        ...
        self._parent._sent[key] = (response, time.time())
        return response
    ...
    def handle_message(self, message):
        ...
        response, timestamp = self._parent._sent.get(key, None)
        if response is None:
        ...
        if message.type == defines.inv_types['RST']:
            for resource in self._parent._relation.keys():
                host, port = message.source
                key = hash(str(host) + str(port) + str(response.token))
                observers = self._parent._relation[resource]
                del observers[key]
        ...
        try:
            call_id, retrasmission_count = self._parent._callID.get(key, None)
