...
class ForwardLayer(object):
    ...
    def receive_request(self, transaction):
        ...
        :rtype : Transaction
        """
        if "/" + transaction.request.uri_path in self._server.root.tree:
            # TODO correct with mapping
            uri_path = transaction.request.uri_path.split("/")
            remote_server = uri_path[0].split(":")
            host = remote_server[0]
            port = int(remote_server[1])
            client = HelperClient((host, port))
            request = Request()
            forward_path = uri_path[1:]
            request.uri_path = "/".join(forward_path)
            request.destination = (host, port)
            request.payload = transaction.request.payload
            # TODO copy safe options
            # request.options =
